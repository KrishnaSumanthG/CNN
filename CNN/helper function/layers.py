...
##COPMUTATIONAL GRAPH
    ...
    #cache=(x_mean,x_var,x_norm,out,bn_param,x,gamma,beta)
    cache=mu,p5,p4,var,p3,p2,p1,x_norm,out1,out,bn_param,x,gamma,beta
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
    ...
    # the out variable.                                                         #
    #############################################################################
    x_norm1=(x-running_mean)/np.sqrt(running_var+eps)
    out=gamma*x_norm1+beta
    cache=(x_norm1,out,bn_param,x,gamma,beta)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  mu,p5,p4,var,p3,p2,p1,x_norm,out1,out,bn_param,x,gamma,beta=cache

  N,D=dout.shape
  dbeta=np.sum(dout,axis=0)
  dout1=dout

  dgamma=np.sum(x_norm*dout1,axis=0)
  dx_norm=gamma*dout1
  
  dp5=p1*dx_norm
  dp1=np.sum(p5*dx_norm,axis=0)

  dp2=-1/(p2**2)*dp1
  dp3=0.5*(p3**(-0.5))*dp2

  dp4=1/float(N)*np.ones((p4.shape))*dp3

  dp5 += 2*p5*dp4

  dx=dp5
  dmu=-np.sum(dp5,axis=0)
  dx += 1/float(N)*np.ones((dp5.shape))*dmu
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for n in xrange(N):
    for f in xrange(F):
      ...
      for m1 in xrange(M1):
        for m2 in xrange(M2):
          out[n,f,m1,m2]=np.sum(x_padded[n,:,s1:HH+s1,s2:WW+s2]*w[f,:])+b[f]
          s2+=S
        s1+=S
        s2=0
  #############################################################################
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in range(N):
    for c in range(C):
      ...
      for k in range(H1):
        for l in range(W1):
          out[n, c, k, l] = np.max(x[n, c, s1:s1 + Hp, s2:s2 + Wp])
          s2+=S
        s1+=S
        s2=0
  #############################################################################
